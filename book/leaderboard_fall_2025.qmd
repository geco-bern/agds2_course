# Leaderboard LULC {#sec-leaderboard}

## Fall 2025

This is the leaderboard for the fall of 2025 Land-Use and Land-Cover competition (complementing the Land-Use and Land-Cover mapping chapter). Submissions are required, but the competition is amicable as rankings will only be considered in the general context of grading. Scoring at the level or better than the base model as demonstrated in the course is however required.
...
if(!ON_GIT) {
  ...
  #   quote = FALSE
  # )
  # # Use random guess as reference for a mock table:
...
files <- list.files(
  here::here("data/leaderboard/fall_2025"),
  "*.csv",
  full.names = TRUE
  )
# loop over all files
  ...
  kbl(
    ...
    footnote(
      ifelse(ON_GIT,
             ...
             "This table was manually generated!"
             )
    )
}
```
